...
class TestResolveEnvVars:
    ...
    def test_provider_key_reference_resolves_or_stays_missing(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.delenv("SCENARIO_PROVIDER_KEY", raising=False)
        provider = ProviderConfig(
            name="test",
            protocol="openai",
            base_url="http://localhost",
            model="test",
            api_key="${SCENARIO_PROVIDER_KEY}",
        )
        assert provider.resolve_api_key() == ""

        monkeypatch.setenv("SCENARIO_PROVIDER_KEY", "secret")
