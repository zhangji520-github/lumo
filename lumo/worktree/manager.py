...
from lumo.worktree.changes import (
    CleanupResult,
    Changes,
    count_worktree_changes,
    has_worktree_changes,
)
from lumo.worktree.models import Worktree, WorktreeSession
from lumo.worktree.session import load_worktree_session, save_worktree_session
from lumo.worktree.setup import perform_post_creation_setup
from lumo.worktree.slug import flatten_slug, validate_slug
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".lumo" / "worktrees"
        )
        self._lumo_dir = Path(repo_root) / ".lumo"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._lumo_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._lumo_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._lumo_dir)
        if session is None:
            return None
