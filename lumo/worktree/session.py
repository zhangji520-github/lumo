...
def _session_path(lumo_dir: Path) -> Path:
    return lumo_dir / SESSION_FILENAME
...
def load_worktree_session(lumo_dir: Path) -> WorktreeSession | None:
    path = _session_path(lumo_dir)
    if not path.exists():
        return None
    try:
