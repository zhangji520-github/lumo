...
"""从 .lumo/commands/ 目录加载自定义 Markdown 命令（对齐 Go 版 loader.go）。
...
def load_user_commands(work_dir: str) -> list[Command]:
    ...
    搜索路径（后者覆盖前者）：
      1. ~/.lumo/commands/
      2. <work_dir>/.lumo/commands/
    """
    dirs: list[str] = []
    home = Path.home()
    dirs.append(str(home / ".lumo" / "commands"))
    dirs.append(str(Path(work_dir) / ".lumo" / "commands"))
