...
async def handle_permission(ctx: CommandContext) -> None:
    ...
    if sub == "":
        ...
        if mode is None:
            ...
            return
        previous = ctx.agent.permission_mode
        ctx.agent.set_permission_mode(mode)
        ctx.ui.refresh_status()
        show_transition = getattr(ctx.ui, "show_mode_transition", None)
        if show_transition is not None:
            show_transition(previous, mode)
        else:
        ...
        if not rule_str:
            ...
            return
        from lumo.permissions.rules import Rule, parse_rule
