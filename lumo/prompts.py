...
from lumo.runtime.prompt_assembler import assemble_runtime_prompt
...
IDENTITY_SECTION = PromptSection(
    ...
    content=(
        ...
        "Do not claim unavailable capabilities or guess the configured model provider."
    ),
)
...
def build_system_prompt(
    ...
    if runtime_context is not None:
        ...
        if coordinator_mode:
            from lumo.teams.coordinator import get_coordinator_system_prompt
            coordinator = get_coordinator_system_prompt(agent_catalog=agent_catalog)
            project = f"{project}\n\n{coordinator}".strip()
    ...
    if persona:
        b.add(PromptSection(
            name="ScenarioPersona",
            priority=10,
            content=f"# Scenario Persona\n\n{persona}",
        ))
