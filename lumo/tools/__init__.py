...
from lumo.tools.base import Tool


@dataclass(frozen=True)
class ToolOrigin:
    kind: Literal["builtin", "pack", "mcp", "cli", "plugin"] = "builtin"
    provider_id: str = "lumo"
...
class ToolRegistry:
    ...
    def register(self, tool: Tool, origin: ToolOrigin | None = None) -> None:
        if tool.name in self._tools:
            existing = self._origins[tool.name]
            incoming = origin or ToolOrigin()
            raise ValueError(
                f"Tool '{tool.name}' from {incoming.kind}:{incoming.provider_id} "
                f"conflicts with {existing.kind}:{existing.provider_id}"
            )
        self._tools[tool.name] = tool
        self._origins[tool.name] = origin or ToolOrigin()
    ...
    def list_registered(self) -> list[tuple[Tool, ToolOrigin]]:
        return [(tool, self._origins[name]) for name, tool in self._tools.items()]
