...
from lumo.skills.parser import SkillDef, SkillParseError, parse_skill_file

log = logging.getLogger(__name__)

PROJECT_SKILLS_DIR = ".lumo/skills"
USER_SKILLS_DIR = "~/.lumo/skills"
...
class SkillLoader:
    ...
    def _selected(self, name: str) -> bool:
        return (
            any(fnmatch(name, pattern) for pattern in self._include)
            and not any(fnmatch(name, pattern) for pattern in self._exclude)
        )
    ...
    def load_all(self) -> dict[str, SkillDef]:
        ...
        seen = {name: skill for name, skill in seen.items() if self._selected(name)}
        self._skills = seen
