...
def get_auto_mem_path(project_root: str) -> str:
    """返回项目级记忆目录路径：<projectRoot>/.lumo/memory/。

    保留尾部分隔符，确保前缀匹配不会误命中类似 memoryxyz 的路径。
    支持 LUMO_REMOTE_MEMORY_DIR 环境变量覆盖。
    """
    override = os.environ.get("LUMO_REMOTE_MEMORY_DIR", "") or os.environ.get(
        "MEWCODE_REMOTE_MEMORY_DIR", ""
    )
    if override:
        return override.rstrip(os.sep) + os.sep
    abs_root = os.path.abspath(project_root)
    return os.path.join(abs_root, ".lumo", "memory") + os.sep


def get_user_auto_mem_path() -> str:
    """返回用户级记忆目录路径：~/.lumo/memory/。
    ...
    if not home:
        return ""
    return os.path.join(home, ".lumo", "memory") + os.sep
...
class MemoryManager:
        ...
        *,
        ...
        if namespace:
            ...
            if not re.fullmatch(r"[a-z0-9][a-z0-9-]*", namespace):
                raise ValueError("memory namespace must be a valid scenario id")
            self._user_mem_dir = os.path.join(
                self._user_mem_dir, "scenarios", namespace
            )
            self._mem_dir = os.path.join(
    ...
    def user_path(self) -> Path:
        ...
        return Path.home() / ".lumo" / "memory" / ENTRYPOINT_NAME
    ...
    def user_mem_dir(self) -> Path:
        """用户级记忆目录（~/.lumo/memory/）。"""
        return Path(self._user_mem_dir.rstrip(os.sep)) if self._user_mem_dir else Path.home() / ".lumo" / "memory"

    @property
    def project_mem_dir(self) -> Path:
        """项目级记忆目录（<project>/.lumo/memory/）。"""
