...
def load_instructions(project_root: str) -> str:
    ...
    发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.lumo/LUMO.md, ~/.lumo/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 LUMO.md 和 AGENTS.md
    3. workDir/.lumo/INSTRUCTIONS.md（遗留格式）
    4. workDir/LUMO.local.md（本地覆盖）
    """
    ...
    def _add_preferred(current: Path, legacy: Path) -> None:
        _add(current if current.exists() else legacy)

    # 1. 用户全局
    _add_preferred(home / ".lumo" / "LUMO.md", home / ".mewcode" / "MEWCODE.md")
    _add_preferred(home / ".lumo" / "AGENTS.md", home / ".mewcode" / "AGENTS.md")

    # 2. 项目目录链
    for d in _project_instruction_dirs(root):
        _add_preferred(d / "LUMO.md", d / "MEWCODE.md")
        _add(d / "AGENTS.md")

    # 3. 遗留格式
    _add_preferred(
        root / ".lumo" / "INSTRUCTIONS.md",
        root / ".mewcode" / "INSTRUCTIONS.md",
    )

    # 4. 本地覆盖
    _add_preferred(root / "LUMO.local.md", root / "MEWCODE.local.md")
