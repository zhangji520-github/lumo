...
from lumo.client import LLMClient, create_client
from lumo.config import AppConfig, ProviderConfig
...
from lumo.runtime.capabilities import ActiveCapabilities, activate_external_capabilities
...
class RuntimeBuilder:
    """Build the shared, surface-independent part of a Lumo runtime."""
        ...
        *,
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".lumo" / "permissions.yaml",
                project_rules_path=Path(self.work_dir) / ".lumo" / "permissions.yaml",
                local_rules_path=Path(self.work_dir) / ".lumo" / "permissions.local.yaml",
            ),
            mode=mode,
    ...
    def _attach_os_sandbox(
        ...
        if bash_tool is None:
            return
        from lumo.sandbox import SandboxConfig, create_sandbox
        ...
        bash_tool.sandbox_config = SandboxConfig(
            ...
            deny_write=[
                f"{self.work_dir}/.lumo/config.yaml",
                f"{self.work_dir}/.lumo/permissions.local.yaml",
            ],
