...
from lumo.runtime.models import CapabilityKind, ScenarioExperience
...
class ScenarioDesignerService:
    ...
    async def propose(self, description: str, client, protocol: str) -> ScenarioProposal:
        ...
        system = """\
You are the Lumo Scenario Designer. Convert the user's goal into a structured
...
workflow and output only; it must not repeat Lumo kernel or tool protocol rules.
...
""" + "\n".join(catalog_lines)
    ...
    async def refine(
        ...
        system = """\
You are refining an existing Lumo scenario. Call SubmitScenarioPatch exactly
