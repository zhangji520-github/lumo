...
class ProviderConfig:
    ...
    def resolve_api_key(self) -> str:
        if self.api_key:
            ...
            # An unresolved environment reference must behave like a missing
            # key instead of being sent as a literal credential.
...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    def config_dir(root: Path) -> Path:
        current = root / ".lumo"
        legacy = root / ".mewcode"
        if (current / "config.yaml").exists() or (current / "config.local.yaml").exists():
            return current
        return legacy if legacy.exists() else current

    user_dir = config_dir(home)
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .lumo/config.yaml "
            "in project or ~/.lumo/config.yaml (legacy .mewcode is also supported)"
        )
    return merged
